#!/usr/bin/python3
"""
@isBinary: if it is set to 1 give header value as hex, useful to find various binary file. 
if isBinary is 0 then string match will be used to match header value.

@urlCount: this will specify how many urls to get from google search.

@example: python find_example.py xml '<?xml' ./xml_corpus 0 100
...
class CSamplesFinder:
    ...
    def find(self, ext, magic, folder, isBinary, count, search=""):
        ...
        url = "https://www.google.com/search?q=filetype:%s+%s+-facebook.com&num=%s" % (
            ext, search, count)
                ...
                if pos > -1:
                    ...
                    try:
                        ...
                        if len(file_data) > MAX_SIZE:
                            log("Discarding file as it's bigger than maximum size (%d kbs)"
                                % (MAX_SIZE / 1024))
                        ...
                        if isBinary == 1:
                            ...
                            log(FileHeader)
                            if magic != FileHeader:
                                log("Discarding file as it doesn't start with %s (starts with %s)"
                                    % (repr(magic), repr(file_data[:5])))
                                continue
                        else:
                        ...
                        f = open(
                            os.path.join(folder, file_hash) + "." + ext, "wb")
                        f.write(file_data)
                        f.close()
...
def usage():
    print("Usage:", sys.argv[0],
          "extension magic_header folder isBinary urlCount")
